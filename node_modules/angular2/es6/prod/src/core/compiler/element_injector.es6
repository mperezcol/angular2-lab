import { isPresent, isBlank, BaseException, stringify, CONST_EXPR } from 'angular2/src/facade/lang';
import { ObservableWrapper } from 'angular2/src/facade/async';
import { ListWrapper, MapWrapper } from 'angular2/src/facade/collection';
import { Injector, Key, Dependency, Binding, ResolvedBinding, NoBindingError, AbstractBindingError, CyclicDependencyError, resolveForwardRef, resolveBindings } from 'angular2/di';
...
export class DependencyWithVisibility extends Dependency {
    constructor(key, asPromise, lazy, optional, properties, visibility) {
...
export class DirectiveDependency extends DependencyWithVisibility {
    constructor(key, asPromise, lazy, optional, properties, visibility, attributeName, queryDirective) {
...
export class DirectiveBinding extends ResolvedBinding {
    constructor(key, factory, dependencies, providedAsPromise, resolvedAppInjectables, resolvedHostInjectables, resolvedViewInjectables, metadata) {
...
export class BindingData {
    ...
    createEventEmitterAccessors() {
        if (!(this.binding instanceof DirectiveBinding))
            return [];
        var db = this.binding;
        return ListWrapper.map(db.eventEmitters, eventName => new EventEmitterAccessor(eventName, reflector.getter(eventName)));
    }
...
export class ProtoElementInjector {
    constructor(parent, index, bd, distanceToParent, _firstBindingIsComponent) {
        this.parent = parent;
        this.index = index;
    ...
    static create(parent, index, bindings, firstBindingIsComponent, distanceToParent) {
        var bd = [];
    ...
    static _createDirectiveBindingData(bindings, bd, firstBindingIsComponent) {
        if (firstBindingIsComponent) {
    ...
    static _createHostInjectorBindingData(bindings, bd) {
    ...
    static _createViewInjectorBindingData(bindings, bd) {
        var db = bindings[0];
        ListWrapper.forEach(db.resolvedViewInjectables, b => ListWrapper.push(bd, new BindingData(ProtoElementInjector._createBinding(b), SHADOW_DOM)));
    }
    static _createBinding(b) {
    ...
    get hasBindings() { return this._strategy.hasBindings(); }
 ...
 */
class _ProtoElementInjectorInlineStrategy {
    constructor(protoEI, bd) {
    ...
    hasBindings() { return isPresent(this._binding0); }
    ...
    createElementInjectorStrategy(ei) {
 ...
 */
class _ProtoElementInjectorDynamicStrategy {
    constructor(protoInj, bd) {
        var len = bd.length;
    ...
    hasBindings() { return isPresent(this._bindings[0]); }
    getBindingAtIndex(index) {
...
export class ElementInjector extends TreeNode {
    ...
    hydrate(injector, host, preBuiltObjects) {
        var p = this._proto;
        this._host = host;
    ...
    _isComponentKey(key) { return this._strategy.isComponentKey(key); }
    ...
    _getByKey(key, visibility, optional, requestor) {
        var ei = this;
    ...
    _getObjByKeyId(keyId, visibility) {
        return this._strategy.getObjByKeyId(keyId, visibility);
    }
    getDirectiveAtIndex(index) { return this._strategy.getDirectiveAtIndex(index); }
...
class ElementInjectorInlineStrategy {
    constructor(_protoStrategy, _ei) {
    ...
    getMaxDirectives() { return _MAX_DIRECTIVE_CONSTRUCTION_COUNTER; }
}
/**
 * Strategy used by the `ElementInjector` when the number of bindings is 10 or less.
 * In such a case, inlining fields is benefitial for performances.
...
class ElementInjectorDynamicStrategy {
    ...
    clearInstances() { ListWrapper.fill(this._objs, null); }
    hydrate() {
        var p = this._protoStrategy;
    ...
    getComponent() { return this._objs[0]; }
    isComponentKey(key) {
        return this._ei._proto._firstBindingIsComponent && isPresent(key) &&
    ...
    getMaxDirectives() { return this._objs.length; }
}
